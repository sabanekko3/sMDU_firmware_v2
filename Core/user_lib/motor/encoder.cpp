...
sincos_t AS5600::get_e_sincos(void){
	...
	data.sin = motor_math::sin_table(angle);
	data.cos = motor_math::cos_table(angle);
}
...
uint16_t AB_LINER::get_e_angle(void){
	...
	return motor_math::fast_atan2_angle(sincos_data.sin,sincos_data.cos);
}
int32_t AB_LINER::get_e_angle_sum(void){
	sincos_t sincos_data = get_e_sincos();
	return (float)check_turn(sincos_data)*1024 + motor_math::fast_atan2_angle(sincos_data.sin,sincos_data.cos);
}
