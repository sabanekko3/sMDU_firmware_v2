...
public:
	virtual void init() = 0;
	virtual void rotate(float rad_per_s);
};


typedef struct BLDCMotor_element{

};

class BLDC_motor:motor{
private:
	DRIVER *driver = NULL;
	ANALOG_SENS *analog = NULL;
	ENCODER *enc = NULL;
...
public:
	MOTOR(DRIVER *_driver,ANALOG_SENS *_analog)
		:driver(_driver),analog(_analog),
